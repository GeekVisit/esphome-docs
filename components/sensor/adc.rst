...
You can only use as many ADC sensors as your device can support.  The Esp8266 only has one ADC and can only handle one channel at a time. On the ESP8266, you can measure an analog pin (A0 on ESP8266) or VCC (see below) but NOT both simultaneously. Using both at the same time will result in incorrect sensor values.
